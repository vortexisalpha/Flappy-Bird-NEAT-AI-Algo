...
class Bird:
    ...
    def draw(self, win):
        #basically every five frames rotate image 
        self.img_count += 1
        ...
        #dont flap wings if its diving down
        if self.tilt <= -80:
            self.img = self.IMGS[1]
            self.img_count = self.ANIMATION_TIME*2

        #rotate the image around the center based on the tilt
...
class Pipe:
    ...
    def collide(self, bird): #MASKS : pixel perfect collision
        ...
        b_point = bird_mask.overlap(bottom_mask, bottom_offset) #returns none on no collision
        t_point = bird_mask.overlap(top_mask, top_offset) #same

        if t_point or b_point: #if not none
            return True
        else: return False

class Base: #create 2 images and keep cycling when one goes off screen
    VEL = 5 #same as pipe 
...
def draw_window(win,birds, pipes, base, score):
    win.blit(BG_IMG, (0,0)) #draw background in top left
    for pipe in pipes: 
        pipe.draw(win)
...
def main(genomes, config):
    global gen
    gen+=1
    nets =  [] #for keeping track of neaural network
    ge = [] #for keeping track of genomes
    birds = []

    for _, g in genomes:
        net = neat.nn.FeedForwardNetwork.create(g,config)# create neural network for bird
        nets.append(net) # append to neural network list
    ...
    while run:
        ...
        for x, bird in enumerate(birds):
            # every frame the bird will get 0.1 points for going forward
            bird.move()
            ge[x].fitness += 0.1
        ...
        for pipe in pipes:
            for x, bird in enumerate(birds):
                if pipe.collide(bird): #then remove the bird from existance
